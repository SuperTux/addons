...
if((! ("iceberg_bonus" in state)) || (! ("iceberg_reveal" in state))){
  ...
  print("[DEBUG] bonuses for mattie's iceberg initialized\n");
}
else {
  foreach(name in bonus){
    if(! (name in state.iceberg_bonus)){
      state.iceberg_bonus[name] <- false;
      print("[DEBUG] compensated for mattie's iceberg old profile format\n");
    }
...
if(! ("easy" in state)){state.easy <- false;}
...
function set_time(level){
  if(level == "mattie_iceberg/L_DOOM.stl")
    ...
    temp <- 39;
  else if(level == "mattie_iceberg/L_MtnPass.stl")
    temp <- 75;
  else
    temp <- 44;
...
function show_progress(name, type){
  ...
  if(name == "mattie_iceberg/L_MtnPass.stl" && type == "coins-collected")
    coin11.set_action("solved");
  if(name == "mattie_iceberg/L_MtnPass.stl" && type == "badguys-killed")
    kill11.set_action("solved");
  if(name == "mattie_iceberg/L_MtnPass.stl" && type == "time-needed")
