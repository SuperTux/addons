...
archtext <- {a="1\nplay the first level"
             ...
             g="7\nplay the bonus level in icy sequence"
             h="8\nfind the secret level in icy sequence"
             i="9\nbuild the higherst totem"
             j="10\nplay a level in icy sequence"
             k="11\nkill a yeti"
             l="12\nuse the deutor trough crystal sequence"
             m="13\nplay a level in crystal sequence"
             n="14\nplay the bonus level in crystal sequence"
             o="15\nfind the secret level in crystal sequence"
             p="16\ncomplete the deutor trough crystal sequence"
             q="17\nfind a secret platform"
             r="18\ndouble score"
             s="19\nplay a level in dark-forest sequence"
             t="20\nplay the bonus level in dark-forest sequence"
             u="21\nfind the secret level in dark-forest sequence"
             v="22\ndo trough the boswer in dark-forest sequence"
             w="23\nreach the impossible-hard level"
             x="24\npass, but not destroy"
             y="25\nplay a level in water sequence"
             ...
             af="32\nplay a level in secret sequence"
             ag="33\nplay the non-secret bonus level in secret sequence"
             ah="34\nfind the secret level in secret sequence"
             ai="35\nbest survivor"
             aj="36\nI don't want bonus"}
...
function unlock_archivement(archivement){
 if (! (archivements[archivement])){
  ...
  Text.set_pos(120,30);
  Text.set_text("Archivement ulocked! #"+archtext[archivement]);
  Text.fade_in(0.2);
  save_archivements();
